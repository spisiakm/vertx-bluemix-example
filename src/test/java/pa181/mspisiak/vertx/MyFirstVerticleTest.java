...
public class MyFirstVerticleTest {
	...
	public void checkThatTheIndexPageIsServed(TestContext context) {
		...
		vertx.createHttpClient().getNow(port, "localhost", "/assets/index.html", response -> {
			...
			response.bodyHandler(body -> {
				context.assertTrue(body.toString().contains("<title>My Book Collection</title>"));
				async.complete();
			});
		});
	}
	...
	public void checkThatWeCanAdd(TestContext context) {
		Async async = context.async();
		final String json = Json.encodePrettily(new Book("Space", "Aliens"));
		vertx.createHttpClient().post(port, "localhost", "/api/books")
				...
				.handler(response -> {
					...
					response.bodyHandler(body -> {
						final Book book = Json.decodeValue(body.toString(), Book.class);
						context.assertEquals(book.getName(), "Space");
						context.assertEquals(book.getAuthor(), "Aliens");
						context.assertNotNull(book.getId());
						async.complete();
					});
				})
				.write(json)
